...
namespace Coverlet.Core.Instrumentation
{
  ...
  internal class Instrumenter
  {
    ...
    private void InstrumentIL(MethodDefinition method)
    {
      ...
      // outside all exception handlers. IL validation rules prohibit a branch instruction inside an
      // exception handler from jumping to code outside it, and vice versa. For those branches we
      // fall back to the original inline counter insertion.
      var insideExceptionHandlerOffsets = new HashSet<int>();
      if (method.Body.HasExceptionHandlers)
      {
        foreach (Instruction bodyInstr in processor.Body.Instructions)
        {
          int off = bodyInstr.Offset;
          foreach (ExceptionHandler eh in method.Body.ExceptionHandlers)
          {
            if ((eh.TryStart != null && eh.TryEnd != null &&
                   off >= eh.TryStart.Offset && off < eh.TryEnd.Offset) ||
                  (eh.TryStart != null && eh.TryEnd == null &&
                   off >= eh.TryStart.Offset) ||
                  (eh.HandlerStart != null && eh.HandlerEnd != null &&
                   off >= eh.HandlerStart.Offset && off < eh.HandlerEnd.Offset) ||
                  (eh.HandlerStart != null && eh.HandlerEnd == null &&
                   off >= eh.HandlerStart.Offset) ||
                  (eh.FilterStart != null && eh.HandlerStart != null &&
                   off >= eh.FilterStart.Offset && off < eh.HandlerStart.Offset))
            {
              insideExceptionHandlerOffsets.Add(off);
              break;
            }
          }
        }
      ...
      // Snapshot of original instructions before any insertions. ReplaceInstructionTarget must
      // only iterate over these: newly-inserted counter/trampoline instructions have a stale
      // Offset of 0 (Cecil does not recalculate offsets until OptimizeMacros), which would cause
      // targetsMap[0] to be looked up and the trampoline's `br` to be redirected to the counter
      // for the first original instruction instead of its intended target.
      List<Instruction> originalInstructions = [.. processor.Body.Instructions];
      ...
      for (int n = 0; n < count; n++)
      {
        ...
        foreach (BranchPoint branchTarget in targetedBranchPoints)
        {
          ...
          if (branchTarget.Path == 0)
          {
            // Fall-through (default) path: insert the counter inline before the target instruction.
            // This is the path reached by sequential execution when the branch condition is not taken.
          ...
          {
            // Taken-branch (jump) path: prefer a trampoline appended at the end of the method body
            // so the counter fires only when the specific conditional jump is taken, not on any
            // fall-through arrival at the same instruction from another path. This prevents the
            // false-positive branch coverage that affected `if` without `else`.
            //
            // A trampoline is only useful when the jump target is ALSO reachable by sequential
            // fall-through from the preceding instruction. When the preceding instruction is an
            // unconditional branch, return, or throw (FlowControl.Branch/Return/Throw), the target
            // can only be reached via the specific jump, so no false-positive is possible and
            // inline insertion is sufficient.
            //
            // Trampolines also live outside all exception handler regions. A branch instruction
            // inside an exception handler cannot jump to code outside it (and vice versa), because
            // the CLR considers that invalid IL. When either the branch instruction or its jump
            // target is inside an exception handler region, fall back to inline insertion.
            bool targetReachableByFallThrough = n > 0 &&
                originalInstructions[n - 1].OpCode.FlowControl is not
                    (FlowControl.Branch or FlowControl.Return or FlowControl.Throw);

            bool crossesExceptionBoundary =
                insideExceptionHandlerOffsets.Contains(branchTarget.Offset) ||
                insideExceptionHandlerOffsets.Contains(branchTarget.EndOffset);

            if (crossesExceptionBoundary || !targetReachableByFallThrough)
            {
              // Inline fallback: preserves valid IL. Counter is inserted before the jump target so
              // it fires on every arrival, which may produce false-positive counts for patterns like
              // `if` without `else` inside exception-handler regions (e.g. async state machines).
              Instruction firstInjectedInstrumentedOpCode = AddInstrumentationCode(method, processor, currentInstruction, branchTarget);
              if (!targetsMap.ContainsKey(currentInstruction.Offset))
                targetsMap.Add(currentInstruction.Offset, firstInjectedInstrumentedOpCode);
      ...
      foreach (Instruction bodyInstruction in originalInstructions)
        ReplaceInstructionTarget(bodyInstruction, targetsMap, branchRedirectMap);
    ...
    private void EnsureCustomTrackerRecordHitMethod(MethodDefinition method)
    {
      if (_customTrackerRecordHitMethod is not null)
        return;
    ...
    private Instruction AddBranchTrampolineCode(MethodDefinition method, ILProcessor processor, BranchPoint branchPoint, Instruction trampolineTarget)
    {
      if (!_result.Documents.TryGetValue(_sourceRootTranslator.ResolveFilePath(branchPoint.Document), out Document document))
      {
        document = new Document
      ...
      EnsureCustomTrackerRecordHitMethod(method);

      // Append a self-contained trampoline block at the end of the method body:
      //   ldc.i4  <hitEntryIndex>
      //   call    RecordHit
      //   br      <trampolineTarget>   ← jump to the normal entry point of the destination
      // The trampoline is only reachable when the specific conditional jump is taken, so
      // its counter does not fire on fall-through arrivals at the same destination.
      var indxInstr = Instruction.Create(OpCodes.Ldc_I4, _result.HitCandidates.Count - 1);
      var callInstr = Instruction.Create(OpCodes.Call, _customTrackerRecordHitMethod);
      var brInstr = Instruction.Create(OpCodes.Br, trampolineTarget);

      processor.Append(indxInstr);
      processor.Append(callInstr);
      processor.Append(brInstr);
        ...
        IDictionary<(int branchOffset, int path), Instruction> branchRedirectMap)
    {
      if (instruction.Operand is Instruction operandInstruction)
      {
        ...
        else if (targetsMap.TryGetValue(operandInstruction.Offset, out Instruction newTarget))
        {
